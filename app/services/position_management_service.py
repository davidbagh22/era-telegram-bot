...
async def list_applications_for_office(session: AsyncSession, office_id: int) -> list[PositionApplication]:
    return list(
        (
            await session.scalars(
                select(PositionApplication)
...
async def submit_application(
    ...
    *,
    ...
    locked_office = await session.scalar(select(Office).where(Office.id == office.id).with_for_update())
    ...
    if office_management_service.normalize_recruitment_mode(getattr(locked_office, "recruitment_mode", None)) == office_management_service.RECRUITMENT_CLOSED:
        raise PositionError("applications_closed")
    ...
    existing = await session.scalar(
        select(PositionApplication).where(
            ...
            PositionApplication.status.in_(list(_OPEN_STATUSES)),
        )
    )
    ...
    application = PositionApplication(
        ...
        relevant_experience=(relevant_experience or "").strip()[:4000] or None,
        plan=(plan or "").strip()[:4000] or None,
        availability=(availability or "").strip()[:100] or None,
        attachment_url=(extra or "").strip()[:1000] or None,
...
async def withdraw_application(session: AsyncSession, application: PositionApplication, *, user_id: int) -> None:
...
async def review_application(
    ...
    *,
    ...
    if application.status in (PositionApplicationStatus.WITHDRAWN, PositionApplicationStatus.APPOINTED):
    ...
    await audit(
        ...
        entity_id=application.id,
        old_value=None,
        new_value={"status": status, "has_note": bool(note)},
    )
    return application
...
async def end_appointment(
    ...
    *,
    ...
    office = await session.scalar(select(Office).where(Office.id == assignment.office_id).with_for_update())
...
async def extend_appointment(session: AsyncSession, assignment: UserOffice, *, new_ends_at: date, actor_id: int) -> None:
...
async def candidate_summary(session: AsyncSession, user_id: int) -> CandidateSummary:
    completed_projects = int(
        await session.scalar(
            select(func.count()).select_from(ProjectMember).join(Project, Project.id == ProjectMember.project_id).where(ProjectMember.user_id == user_id, Project.status == ProjectStatus.COMPLETED)
        ) or 0
    )
    tasks_completed_total = int(await session.scalar(select(func.count()).select_from(Task).where(Task.assignee_id == user_id, Task.status == TaskStatus.COMPLETED)) or 0)
    tasks_completed_on_time = int(
        await session.scalar(
            select(func.count()).select_from(Task).where(Task.assignee_id == user_id, Task.status == TaskStatus.COMPLETED, Task.updated_at <= Task.deadline)
        ) or 0
    )
    on_time_rate = round(tasks_completed_on_time / tasks_completed_total * 100, 1) if tasks_completed_total else None
    events_attended = int(
        await session.scalar(
            select(func.count()).select_from(EventRegistration).join(Event, Event.id == EventRegistration.event_id).where(EventRegistration.user_id == user_id, EventRegistration.status == RegistrationStatus.ATTENDED)
        ) or 0
    )
    past_offices = int(await session.scalar(select(func.count()).select_from(UserOffice).where(UserOffice.user_id == user_id, UserOffice.is_active.is_(False))) or 0)
    return CandidateSummary(completed_projects, tasks_completed_on_time, tasks_completed_total, on_time_rate, events_attended, past_offices)
...
def suggested_roles(summary: CandidateSummary) -> list[str]:
    roles: list[str] = []
    if summary.tasks_completed_total >= _CURATOR_MIN_TASKS and (summary.on_time_rate or 0) >= _CURATOR_MIN_ON_TIME_RATE:
...
async def office_history(session: AsyncSession, user_id: int) -> list[OfficeHistoryEntry]:
    rows = (
        await session.execute(
            select(UserOffice, Office).join(Office, Office.id == UserOffice.office_id).where(UserOffice.user_id == user_id).order_by(UserOffice.starts_at.desc())
        )
    ).all()
    return [OfficeHistoryEntry(office.title, assignment.starts_at, assignment.ends_at, assignment.is_active) for assignment, office in rows]
...
async def list_cadre_reserve(session: AsyncSession, *, limit: int = 100) -> list[CadreReserveEntry]:
    already_leading = {
        row[0]
        for row in (
            await session.execute(select(UserOffice.user_id).join(Office, Office.id == UserOffice.office_id).where(UserOffice.is_active.is_(True)))
        ).all()
    }
    candidates = list((await session.scalars(select(User).where(User.is_archived.is_(False), User.is_blocked.is_(False)))).all())
    ...
    for user in candidates:
        ...
        if roles:
            entries.append(CadreReserveEntry(user.id, user.first_name, user.last_name, summary, roles))
