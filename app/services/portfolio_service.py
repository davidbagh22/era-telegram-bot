...
from app.database.models import (
    Badge, Department, Direction, Event, EventActivitySubmission, EventRegistration,
    Office, PortfolioItem, Project, ProjectMember, ProjectRole, Task, TaskSubmission,
    User, UserBadge, UserDepartment, UserDirection, UserOffice,
)
from app.repositories.users import user_stats
from app.utils.constants import (
    EVENT_STATUS_LABELS, PROJECT_STATUS_LABELS, REGISTRATION_STATUS_LABELS,
    ROLE_LABELS, STATUS_LABELS, TASK_STATUS_LABELS,
)
...
def _period(user: User) -> str:
    ...
    return "период участия уточняется" if created_at is None else f"с {_date_label(created_at)}"
...
async def build_portfolio_data(session: AsyncSession, user: User) -> PortfolioData:
    ...
    portfolio_items = list((await session.scalars(
    ...
    projects = list((await session.scalars(
        select(Project).where(Project.author_id == user.id).order_by(desc(Project.created_at), desc(Project.id))
    )).all())
    contribution_rows = list((await session.execute(
        ...
        .order_by(desc(ProjectMember.contribution_confirmed_at), desc(ProjectMember.id))
    )).all())
    departments = list((await session.scalars(
        ...
        .where(UserDepartment.user_id == user.id).order_by(Department.name)
    )).all())
    directions = list((await session.scalars(
        select(Direction.name).join(UserDirection, UserDirection.direction_id == Direction.id)
        .where(UserDirection.user_id == user.id).order_by(Direction.name)
    )).all())
    event_rows = list((await session.execute(
        select(EventRegistration, Event).join(Event, Event.id == EventRegistration.event_id)
        .where(EventRegistration.user_id == user.id).order_by(desc(Event.event_date), desc(Event.id))
    )).all())
    task_rows = list((await session.execute(
        select(TaskSubmission, Task).join(Task, Task.id == TaskSubmission.task_id)
        .where(TaskSubmission.user_id == user.id).order_by(desc(TaskSubmission.created_at), desc(TaskSubmission.id))
    )).all())
    activity_rows = list((await session.scalars(
        select(EventActivitySubmission).where(EventActivitySubmission.user_id == user.id)
        .order_by(desc(EventActivitySubmission.created_at), desc(EventActivitySubmission.id))
    )).all())
    badge_rows = list((await session.execute(
        select(UserBadge, Badge).join(Badge, Badge.id == UserBadge.badge_id)
        .where(UserBadge.user_id == user.id).order_by(desc(UserBadge.created_at), Badge.name)
    )).all())
    office_rows = list((await session.execute(
        select(UserOffice, Office).join(Office, Office.id == UserOffice.office_id)
    ...
    recommendations = [item for item in confirmed_items if item.category in {"recommendation", "leader_recommendation", "partner_recommendation"}]
    ...
    return PortfolioData(
        ...
        projects=_limited(
            [PortfolioEntry(
                title=_clean(project.title), description=_clean(project.short_description),
                status=PROJECT_STATUS_LABELS.get(project.status, _clean(project.status)),
                date_label=_date_label(project.submitted_at or project.created_at), category="project",
                entity_kind="project", entity_id=project.id,
            ) for project in projects]
            + [PortfolioEntry(
                title=_clean(project.title),
                description=_clean(member.contribution_summary) or _clean(member.contribution_result) or _clean(project.short_description),
                status="вклад подтверждён",
                date_label=_date_label(member.contribution_confirmed_at or member.joined_at or project.created_at),
                category=_clean(member.contribution_role_title) or _clean(role.title if role else "") or "project_contribution",
                entity_kind="project", entity_id=project.id,
            ) for member, project, role in contribution_rows]
        ),
        events=_limited([PortfolioEntry(
            title=_clean(event.title), description=_clean(event.description),
            status=REGISTRATION_STATUS_LABELS.get(registration.status, _clean(registration.status)),
            date_label=_date_label(event.event_date), category=EVENT_STATUS_LABELS.get(event.status, _clean(event.status)),
            entity_kind="event", entity_id=event.id,
        ) for registration, event in event_rows]),
        tasks=_limited([PortfolioEntry(
            title=_clean(task.title), description=_clean(submission.text) or _clean(task.description),
            status=TASK_STATUS_LABELS.get(submission.status, _clean(submission.status)),
            date_label=_date_label(submission.created_at), category="task", file_id=submission.file_id,
            entity_kind="task", entity_id=task.id,
        ) for submission, task in task_rows]),
        volunteer=_limited([PortfolioEntry(
            title="Активность после мероприятия", description=_clean(submission.text) or "подтверждение активности",
            status=TASK_STATUS_LABELS.get(submission.status, _clean(submission.status)),
            date_label=_date_label(submission.created_at), category="volunteer", file_id=submission.file_id,
        ) for submission in activity_rows]),
        leadership=_limited([PortfolioEntry(
            title=_clean(office.title), description=_clean(office.description), status="активная роль",
            date_label=_date_label(user_office.starts_at), category="leadership",
        ) for user_office, office in office_rows]),
        badges=_limited([PortfolioEntry(
            title=_clean(badge.name), description=_clean(badge.description) or _clean(user_badge.reason),
            status="подтверждено", date_label=_date_label(user_badge.created_at), category="badge",
        ...
        confirmed_items=_limited(confirmed_items), pending_items=_limited(pending_items),
    )
...
def portfolio_summary_text(data: PortfolioData) -> str:
    ...
    return "\n".join([
        "🎓 Портфолио ЭРА", "", data.full_name, f"Роль: {data.role}",
        f"Статус роста: {data.participation_status}", f"Период участия: {data.period}",
        f"Департамент: {departments}", f"Направление: {directions}", "", "Что уже собрано:",
        f"• мероприятия: {data.stats.get('events', 0)}", f"• проекты: {data.stats.get('projects', 0)}",
        f"• задачи: {data.stats.get('tasks', 0)}", f"• баллы: {data.stats.get('points', 0)}",
        f"• подтверждённые достижения: {len(data.confirmed_items)}", f"• на проверке: {len(data.pending_items)}",
        f"• знаки: {len(data.badges)}", f"• сертификаты: {len(data.certificates)}", "",
