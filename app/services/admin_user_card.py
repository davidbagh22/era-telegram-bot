...
from app.services.notification_service import admin_notification_recipients, safe_answer_photo, safe_send, safe_send_photo
from app.utils.constants import APPLICATION_STATUS_LABELS, PERMISSION_LABELS, ROLE_LABELS, STATUS_LABELS
...
async def _socials(session: AsyncSession, user_id: int) -> tuple[str | None, str]:
    ...
    links = (await session.scalars(select(SocialLink).where(SocialLink.user_id == user_id, SocialLink.is_active.is_(True)).order_by(SocialLink.platform, SocialLink.url))).all()
...
async def _activity_summary(session: AsyncSession, target: User) -> tuple[int, int, str]:
    points = int(await session.scalar(select(func.coalesce(func.sum(PointTransaction.points), 0)).where(PointTransaction.user_id == target.id)) or 0)
    portfolio_count = int(await session.scalar(select(func.count()).select_from(PortfolioItem).where(PortfolioItem.user_id == target.id)) or 0)
...
def _permissions(target: User) -> str:
    active = [grant.permission for grant in (getattr(target, "permission_grants", None) or []) if grant.is_active]
    return ", ".join(PERMISSION_LABELS.get(item, item) for item in sorted(active)) or "нет отдельных прав"

def _application_miniapp_markup(reply_markup: InlineKeyboardMarkup, miniapp_url: str, user_id: int) -> InlineKeyboardMarkup:
    if not miniapp_url:
        return reply_markup
    url = miniapp_path_url(miniapp_url, "admin", {"adminSection": "applications", "applicationId": user_id})
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📲 Открыть заявку в приложении", web_app=WebAppInfo(url=url))], *reply_markup.inline_keyboard])

async def build_admin_user_card(session: AsyncSession, target: User, *, mode: str = "profile") -> AdminUserCard:
    ...
    base = [("Имя", _name(target)), ("Telegram", _telegram(target)), ("Дата рождения", _date_text(getattr(target, "birth_date", None))), ("Возраст", str(target.age) if target.age else None), ("Несовершеннолетний (по анкете)", _minor_label(target.age)), ("Город", target.city), ("Телефон", target.phone), ("Email", target.email)]
    structure = [("Роль", _label(ROLE_LABELS, target.role)), ("Статус", _label(STATUS_LABELS, target.participation_status)), ("Заявка", _label(APPLICATION_STATUS_LABELS, target.application_status)), ("Блокировка", "да" if target.is_blocked else "нет"), ("Архив", "да" if target.is_archived else "нет"), ("Департаменты", _departments(target)), ("Направления", _directions(target))]
    activity = [("Баланс", f"{points} баллов"), ("Портфолио", str(portfolio_count)), ("Знаки", badge_names), ("Права", _permissions(target))]
    if mode == "application":
        title = f"📝 Заявка #{target.id}"
        skills = ", ".join(getattr(target, "skills", None) or []) or "не указаны"
        extra = "\n\n" + _lines("Что важно для решения", [("Учёба / работа", target.education_work), ("Занятие", target.occupation), ("Навыки", skills), ("Опыт", getattr(target, "experience", None)), ("Доступное время", target.available_time), ("Желаемый путь", target.desired_path), ("Согласие на обработку данных", "получено" if target.personal_data_consent else "не получено"), ("Дата подачи", _created_at_text(target))]) + f"\n\nСоцсети\n{social_text}" + f"\n\nМотивация\n{target.motivation or 'не указана'}"
    ...
    text = f"{title}{photo_note}\n\n" + _lines("Профиль", base) + "\n\n" + _lines("ЭРА", structure) + "\n\n" + _lines("Активность", activity) + extra
    return AdminUserCard(text=text, photo_file_id=photo_file_id, reply_markup=reply_markup)

async def send_admin_user_card(message: Message, session: AsyncSession, target: User, *, mode: str = "profile") -> None:
...
async def send_admin_application_cards(bot: Bot, settings: Settings, session: AsyncSession, target: User) -> tuple[int, int]:
    card = await build_admin_user_card(session, target, mode="application")
    reply_markup = _application_miniapp_markup(card.reply_markup, settings.effective_miniapp_url, target.id)
    recipients = await admin_notification_recipients(settings)
    sent = failed = 0
    for chat_id in recipients:
        if card.photo_file_id:
            await safe_send_photo(bot, chat_id, card.photo_file_id, caption=f"🆕 Новая заявка #{target.id}\n{_name(target)}")
    ...
    if sent == 0:
        logger.error("Application card delivery failed for all recipients user_id=%s", target.id)
        raise RuntimeError("application_card_delivery_failed")
    if failed:
        logger.warning("Application card delivery partially failed user_id=%s failed=%s", target.id, failed)
    return sent, failed
