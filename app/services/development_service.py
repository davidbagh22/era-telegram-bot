...
async def save_checkin(session: AsyncSession, checkin: MonthlyCheckin, answers: dict[str, Any], factors: list[str] | None = None, development_wants: list[str] | None = None) -> MonthlyCheckin:
    ...
    for code, value in answers.items():
        if code not in STATE_DIMENSIONS:
...
def _why_text(state: dict[str, int], delta: dict[str, int]) -> str:
    ...
    label = STATE_LABELS[lowest].lower()
    change = delta.get(lowest)
    if change is None:
        return f"Среди текущих областей именно {label} сейчас требует больше внимания. Поэтому мы предлагаем один небольшой эксперимент, а не общий список советов."
    if change <= -10:
        return f"{STATE_LABELS[lowest]} заметно ниже твоего прошлого Check-in ({change} пунктов). Поэтому фокус выбран на ближайший месяц, а не как постоянная характеристика."
    if change >= 10:
        return f"{STATE_LABELS[lowest]} выросла относительно прошлого Check-in (+{change}), но остаётся самой уязвимой частью текущего снимка. Есть смысл закрепить изменение одним действием."
...
async def create_goal(session: AsyncSession, user_id: int, title: str, experiment: str | None, semantic_tag: str | None, is_custom: bool) -> DevelopmentGoal:
    goal = DevelopmentGoal(user_id=user_id, month=current_month(), title=title.strip(), experiment=experiment.strip() if experiment else None, semantic_tag=semantic_tag, is_custom=is_custom)
    session.add(goal)
    await session.flush()
    return goal
...
async def community_analytics(session: AsyncSession, period_days: int = 30) -> dict[str, Any]:
    ...
    if n < 5:
        return {"sample_size":n,"eligible_profiles":participant_count,"coverage_percent":coverage,"minimum_cohort":5,"suppressed":True,"state":None,"message":"Недостаточно ответов для безопасной групповой аналитики."}
