...
def main_menu(channel_url: str, privileged: bool = False, admin: bool = False) -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(keyboard=rows, resize_keyboard=True, is_persistent=True, input_field_placeholder="Выберите раздел ЭРА")


def main_inline_keyboard(privileged: bool = False, admin: bool = False) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(text="👤 Личный кабинет", callback_data="cabinet:open"), InlineKeyboardButton(text="📅 Афиша", callback_data="events:list")],
        [InlineKeyboardButton(text="💡 Проекты", callback_data="projects:menu"), InlineKeyboardButton(text="⭐ Возможности", callback_data="rewards:menu")],
...
def journey_keyboard(internal_chat_url: str | None = None, external_chat_url: str | None = None) -> InlineKeyboardMarkup:
    ...
    rows = [
        [InlineKeyboardButton(text="👤 Мой профиль", callback_data="cabinet:profile"), InlineKeyboardButton(text="🏆 Мои достижения", callback_data="cabinet:achievements")],
        [InlineKeyboardButton(text="➕ Выбрать направление", callback_data="department:apply:start")],
        [InlineKeyboardButton(text="🎓 Портфолио", callback_data="cabinet:portfolio"), InlineKeyboardButton(text="✅ Мои задачи", callback_data="cabinet:tasks")],
        [InlineKeyboardButton(text="📅 Мои мероприятия", callback_data="cabinet:events"), InlineKeyboardButton(text="🏆 Рейтинг", callback_data="cabinet:rating")],
    ]
    if chat_row:
...
def project_menu_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="➕ Создать проект", callback_data="project:create")],
...
def rewards_keyboard(rewards: Iterable, auctions: Iterable) -> InlineKeyboardMarkup:
    rows = []
    rows.extend([[InlineKeyboardButton(text=f"🎁 {reward.name}", callback_data=f"reward:view:{reward.id}")] for reward in rewards])
