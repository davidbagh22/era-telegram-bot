...
def _backup_text(snapshot: dict) -> str:
    ...
    latest = backups[0]
    verified = latest.get("restore_verified_at") or "нет"
    return (
        "Backup ЭРА\n\n"
        ...
        f"Restore verification: {verified}\n"
...
async def _org_text(session: AsyncSession) -> str:
    ...
    offices = int(
        await session.scalar(select(func.count(Office.id)).where(Office.is_active.is_(True))) or 0
    )
    assignments = int(
        await session.scalar(
            select(func.count(UserOffice.id)).where(UserOffice.is_active.is_(True))
        )
        or 0
    )
    vacancies = int(
