...
from app.keyboards.faq import GENERAL_CHAT_EVENTS_TEXT, GENERAL_CHAT_PROFILE_TEXT, general_chat_navigation_keyboard
...
_GENERAL_CHAT_QUICK_ACTIONS = {
    GENERAL_CHAT_EVENTS_TEXT: ("📅 События ЭРА\n\nАфиша, регистрация и ваши ближайшие события — в приложении.", "📅 Открыть события", miniapp_events_url),
    GENERAL_CHAT_PROFILE_TEXT: ("👤 Ваш профиль ЭРА\n\nБаллы, путь, достижения и портфолио — в одном месте.", "👤 Открыть мой профиль", miniapp_profile_url),
}
...
async def handle_chat_join_request(request: ChatJoinRequest, bot: Bot, settings: Settings, session: AsyncSession) -> None:
    ...
    if decision.allowed:
        if await approve_join_request(bot, request.chat.id, request.from_user.id):
            await close_join_request(session, chat_id=request.chat.id, user_id=request.from_user.id, status="approved", reason="approved")
            await audit(session, actor_id=user.id if user else None, action="chat_access.join_request_approved", entity_type="chat_join_request", new_value={"chat_id": request.chat.id, "telegram_id": request.from_user.id, "chat_key": chat_key})
        return
    if decision.chat_key and decision.pending:
        await remember_join_request(session, chat_id=request.chat.id, user_id=request.from_user.id, chat_key=decision.chat_key, reason=decision.reason)
    elif decision.chat_key:
        await decline_join_request(bot, request.chat.id, request.from_user.id)
        await close_join_request(session, chat_id=request.chat.id, user_id=request.from_user.id, status="declined", reason=decision.reason)
        await audit(session, actor_id=user.id if user else None, action="chat_access.join_request_declined", entity_type="chat_join_request", new_value={"chat_id": request.chat.id, "telegram_id": request.from_user.id, "chat_key": decision.chat_key, "reason": decision.reason})
...
async def moderation_status(message: Message, session: AsyncSession) -> None:
    ...
    await message.reply("Мягкая модерация в этом чате: " + ("включена ✅" if enabled else "выключена") + "\nПраво писать открыто для всех.")


@router.message(F.new_chat_members)
async def welcome_members(message: Message, bot: Bot, settings: Settings, session: AsyncSession) -> None:
    me = await bot.get_me()
    welcomed = [member.first_name for member in message.new_chat_members if not member.is_bot]
    if not welcomed:
        return
...
@router.message(F.text.in_({GENERAL_CHAT_EVENTS_TEXT, GENERAL_CHAT_PROFILE_TEXT}), ~F.chat.type.in_({"private"}))
async def general_chat_quick_navigation(message: Message, bot: Bot, user: User | None, settings: Settings) -> None:
    ...
    if not url:
        return
    await safe_send(bot, user.telegram_id, text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=label, web_app=WebAppInfo(url=url))]]))
