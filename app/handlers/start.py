...
async def show_home(message: Message, user: User, settings: Settings) -> None:
    ...
    await message.answer(
        ...
        reply_markup=main_menu(
            ...
            admin=user.role == Role.ADMIN or any(grant.is_active for grant in (getattr(user, "permission_grants", None) or [])),
        ),
    )
...
async def start(message: Message, bot: Bot, user: User | None, settings: Settings, state: FSMContext) -> None:
    await state.clear()
    ...
    if subscribed is None:
        if _approved_existing_user(user):
            ...
            return
        await message.answer(getattr(texts, "SUBSCRIPTION_CHECK_UNAVAILABLE", "Subscription check is unavailable."), reply_markup=subscription_keyboard(settings.era_channel_url))
        return
    if not subscribed:
...
async def check_subscription(call: CallbackQuery, bot: Bot, user: User | None, settings: Settings, state: FSMContext) -> None:
    await call.answer()
    await state.clear()
    ...
    if subscribed is None:
        if _approved_existing_user(user):
            ...
            return
        await call.message.answer(getattr(texts, "SUBSCRIPTION_CHECK_UNAVAILABLE", "Subscription check is unavailable."), reply_markup=subscription_keyboard(settings.era_channel_url))
        return
    if not subscribed:
