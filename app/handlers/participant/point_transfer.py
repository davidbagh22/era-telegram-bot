...
async def find_user(session: AsyncSession, value: str) -> User | None:
    clean = (value or "").strip().removeprefix("@")
    if clean.isdigit():
        return await session.scalar(select(User).where(or_(User.telegram_id == int(clean), User.id == int(clean))))
    return await session.scalar(select(User).where(User.username == clean))


@router.message(Command("give_points"))
@router.callback_query(F.data == "points:transfer:start")
async def start(event: Message | CallbackQuery, user: User | None, state: FSMContext) -> None:
