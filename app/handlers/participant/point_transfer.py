...
async def _recipient(session: AsyncSession, value: str) -> User | None:
    ...
    if clean.isdigit():
        user = await session.scalar(select(User).where(User.telegram_id == int(clean)))
        if user:
            return user
...
async def transfer_recipient(message: Message, user: User | None, session: AsyncSession, state: FSMContext) -> None:
    ...
    if recipient.id == user.id:
        ...
        return
    await state.update_data(recipient_id=recipient.id, recipient_name=f"{recipient.first_name} {recipient.last_name or ''}".strip())
