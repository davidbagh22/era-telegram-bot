...
Merge commit: `7151d9658cc8d11da0a3351e57543f4b820500c1`.
...
#### Продуктовый блок. Портфолио и PDF-резюме

Проверено:
- текущий путь `Личный кабинет → Портфолио → Скачать портфолио PDF`;
- старые callbacks `portfolio:view`, `portfolio:upload`, `portfolio:resume`;
- сервис PDF `resume_service`;
- текущие данные профиля, департаментов, направлений, проектов, мероприятий, задач, знаков, ролей и `PortfolioItem`;
- обработка пустого портфолио, кириллицы, длинных названий и спецсимволов.

Найдено:
- функция PDF не была удалена полностью, но была спрятана глубоко внутри `Мои данные → Портфолио`;
- экран портфолио и PDF собирали данные разными способами;
- PDF включал слишком узкий набор данных: базовые контакты, счётчики и `PortfolioItem`;
- имя файла содержало внутренний user id;
- ошибка генерации PDF не логировалась достаточно явно;
- текстовая кнопка `Возможности` и inline-кнопка `Возможности` открывали разные уровни раздела.

Сделано:
- добавлен единый `portfolio_service` с нормализованной структурой `PortfolioData`;
- экран портфолио и PDF теперь опираются на один источник данных;
- `Портфолио` возвращено на первый уровень личного кабинета;
- PDF включает роль, статус роста, департаменты, направления, период участия, проекты, мероприятия, задачи, волонтёрскую активность, лидерские роли, достижения, сертификаты, рекомендации, знаки и компетенции;
- PDF больше не показывает технические поля вроде Telegram ID/internal ID/status callbacks;
- имя файла стало человеческим: `ERA_portfolio_<name>.pdf`;
- добавлено короткое состояние загрузки перед генерацией;
- ошибки генерации и отправки логируются, пользователь остаётся в разделе портфолио;
- экспорт использует `BufferedInputFile`, временные локальные файлы не создаются;
- top-level `Возможности` теперь открывают единый хаб `offers:menu`;
- пустая афиша получила кнопку возврата в главное меню.

Тесты:
- добавлен `tests/test_portfolio_resume_export.py`;
- обновлён `tests/test_v2_scenarios.py`.
...
Фактическая карта навигации:
- новый пользователь: `/start` → подписка/регистрация → ожидание approval;
- одобренный участник: главное меню → личный кабинет / афиша / задачи / возможности / связь;
- активный участник: личный кабинет → данные, задачи, портфолио, баллы, рейтинг, мероприятия, проекты;
- лидер: обычный путь + панель лидера для задач, мероприятий, предложений, отчётов и лидерских рассылок;
- администратор: обычный путь + админ-панель с участниками, событиями, общением, развитием, аналитикой и системой.

P0 — мешает пользоваться:
- Проблема: портфолио было скрыто во втором уровне `Мои данные`.
  Текущее поведение: пользователь не видел быстрый экспорт из личного кабинета.
  Предлагаемое поведение: `Портфолио` на первом уровне личного кабинета.
  Польза: экспорт снова находится там, где его ожидают.
  Риск: low.
  Сложность: low.
  Сейчас: внедрено.
- Проблема: пустая афиша не давала явного выхода.
  Текущее поведение: пользователь получал текст без кнопки назад.
  Предлагаемое поведение: добавить `← Главное меню`.
  Польза: нет тупика.
  Риск: low.
  Сложность: low.
  Сейчас: внедрено.

P1 — заметно упрощает UX:
- Проблема: `Возможности` из разных входов открывали разные экраны.
  Текущее поведение: reply-кнопка открывала хаб, inline-кнопка открывала только каталог наград.
  Предлагаемое поведение: все top-level входы ведут в `offers:menu`.
  Польза: партнёрские предложения, заявки, аукционы и каталог не прячутся.
  Риск: low.
  Сложность: low.
  Сейчас: внедрено.
- Проблема: портфолио и PDF собирали данные разными способами.
  Текущее поведение: экран показывал `PortfolioItem`, PDF показывал ограниченный набор.
  Предлагаемое поведение: единый `PortfolioData`.
  Польза: пользователь видит, что войдёт в документ.
  Риск: medium.
  Сложность: medium.
  Сейчас: внедрено с regression tests.
- Проблема: раздел задач уже разделён на активные/общие/архив, но старый cabinet handler с `cabinet:tasks` всё ещё остаётся в коде ниже по порядку.
  Текущее поведение: фактически работает новый flow, но legacy создаёт риск конфликтов при изменении порядка роутеров.
  Предлагаемое поведение: на следующем cleanup удалить или изолировать старый duplicate handler.
  Польза: меньше риска битых callbacks.
  Риск: medium.
  Сложность: medium.
  Сейчас: зафиксировано, не трогать в этом PR без отдельной проверки.

P2 — nice to have:
- Проблема: часть inline flows отправляет новые сообщения вместо редактирования.
  Текущее поведение: активный пользователь может получать длинную цепочку сообщений.
  Предлагаемое поведение: точечно переводить стабильные текстовые экраны на `edit_text`.
  Польза: чище чат.
  Риск: medium, особенно рядом с media.
  Сложность: medium.
  Сейчас: отложено.
- Проблема: термины `Аукцион`, `Каталог возможностей`, `Партнёрские предложения` живут внутри одного раздела, но не всегда объясняется разница.
  Текущее поведение: пользователь понимает общий смысл, но может не сразу понять, где что искать.
  Предлагаемое поведение: короткие subtitle-строки в хабе возможностей.
  Польза: меньше вопросов.
  Риск: low.
  Сложность: low.
  Сейчас: зафиксировано.
- Проблема: есть дублирующие event/task/cabinet handlers после предыдущих стабилизаций.
  Текущее поведение: порядок роутеров удерживает рабочий flow.
  Предлагаемое поведение: отдельный legacy cleanup после media/file sends.
  Польза: проще поддержка и меньше скрытых конфликтов.
  Риск: medium.
  Сложность: medium.
  Сейчас: зафиксировано.
