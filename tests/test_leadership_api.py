...
def _weekly_view(**report_overrides) -> SimpleNamespace:
...
class LeadershipApiTests(unittest.TestCase):
    ...
    def test_submit_report(self) -> None:
        ...
        app = _build_app(_leader(), session)
        client = TestClient(app)
        view = _weekly_view(status="red", needs_help=True)
        with patch(
    ...
    def test_read_current_report_uses_weekly_view(self) -> None:
        session = SimpleNamespace(commit=AsyncMock())
        app = _build_app(_leader(), session)
        client = TestClient(app)
        with patch(
            "app.api.v1.leadership.leadership_weekly_service.ensure_weekly_report",
            new=AsyncMock(return_value=_weekly_view()),
        ):
