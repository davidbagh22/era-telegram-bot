...
def test_access_requires_registration_and_approval_for_join_requests() -> None:
    missing = check_chat_access(None, "general")
    pending = check_chat_access(
        _user(application_status=ApplicationStatus.PENDING), "general"
    )
    rejected = check_chat_access(
        _user(application_status=ApplicationStatus.REJECTED), "general"
    )
    assert missing.allowed is False and missing.pending is True and missing.reason == "not_registered"
    assert pending.allowed is False and pending.pending is True and pending.reason == "not_approved"
    assert rejected.allowed is False and rejected.pending is False and rejected.reason == "rejected"
...
def test_blocked_and_archived_users_are_denied_for_managed_access() -> None:
...
def test_general_chat_default_permissions_are_writable() -> None:
    ...
    assert writable.can_send_documents is True
    assert writable.can_send_voice_notes is True


def test_registration_pin_points_to_era_bot_start() -> None:
    assert "ЭРА теперь в одном боте" in GENERAL_REGISTRATION_PIN_TEXT
    assert "Нажми «Начать»" in GENERAL_REGISTRATION_PIN_TEXT
    assert GENERAL_REGISTRATION_START_URL == "https://t.me/ERA_1bot?start=community"
...
def test_chat_handlers_keep_join_access_without_write_restrictions() -> None:
    ...
    assert "pin_chat_message" in service_source
    assert "InlineKeyboardButton" in service_source
