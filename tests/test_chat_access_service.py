...
from app.services.chat_access_service import access_message, chat_key_for_id, check_chat_access, restricted_permissions, writable_permissions
from app.utils.constants import ApplicationStatus, Role

ROOT = Path(__file__).resolve().parents[1]

def _settings() -> SimpleNamespace:
    return SimpleNamespace(general_chat_id=-1001, internal_department_chat_id=-1002, external_department_chat_id=-1003, leaders_chat_id=-1004, chat_ids={-1001, -1002, -1003, -1004})

def _user(**kwargs) -> User:
    data = {"telegram_id": 10, "first_name": "Тест", "role": Role.PARTICIPANT, "application_status": ApplicationStatus.APPROVED, "is_blocked": False, "is_archived": False, "departments": [], "directions": []}
    data.update(kwargs)
    return User(**data)
