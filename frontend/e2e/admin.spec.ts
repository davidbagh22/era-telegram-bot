...
test("analytics shows explainable ERA PULSE and opens the real records behind each metric", async ({ page }) => {
  await enterAdminWorkspace(page);
  await page.getByRole("button", { name: "Контроль" }).click();
  await page.getByRole("button", { name: /Аналитика/ }).click();

  await expect(page.getByText("ERA PULSE", { exact: true })).toBeVisible();
  await expect(page.getByRole("heading", { name: "Что сделать на этой неделе" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "Все данные" })).toBeVisible();
  await expect(page.getByRole("button", { name: /Полный отчёт XLSX/ })).toBeVisible();
  ...
  await expect(page.getByText("Реальных записей", { exact: true })).toBeVisible();
