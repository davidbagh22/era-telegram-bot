...
function consumeTelegramRoute(): void {
  const url = new URL(window.location.href);
  const externalRoute = url.searchParams.get("eraPath") ?? url.searchParams.get("tgWebAppStartParam");
  if (!externalRoute) return;

  const route = cleanRoute(externalRoute);
...
function routeValue(): string {
  return cleanRoute(window.location.hash.replace(/^#\/?/, ""));
}
...
function parseDeepLink(): DeepLink | null {
  ...
  if (adminProjectMatch) {
    ...
    if (projectId !== null) {
      return { tab: "projects", projectId, activitySection: null, communitySection: null, itemId: null, workspace: "admin" };
    }
  }
  ...
  if (projectMatch) {
    ...
    if (projectId !== null) {
      return { tab: "projects", projectId, activitySection: null, communitySection: null, itemId: null, workspace: null };
    }
  }
  if (route === "projects") return { tab: "projects", projectId: null, activitySection: null, communitySection: null, itemId: null, workspace: null };

  const activityMatch = route.match(/^(tasks|calendar|history)(?:\/(\d+))?$/);
  if (activityMatch) {
    return { tab: "home", projectId: null, activitySection: activityMatch[1] as LegacyActivitySection, communitySection: null, itemId: parseOptionalId(activityMatch[2]), workspace: null };
  }

  const eventMatch = route.match(/^events(?:\/(\d+))?$/);
  if (eventMatch) {
    return { tab: "events", projectId: null, activitySection: null, communitySection: null, itemId: parseOptionalId(eventMatch[1]), workspace: null };
  }

  const communityMatch = route.match(/^(opportunities|auctions|rewards|surveys)(?:\/(\d+))?$/);
  if (communityMatch) {
    return { tab: "community", projectId: null, activitySection: null, communitySection: communityMatch[1] as CommunitySection, itemId: parseOptionalId(communityMatch[2]), workspace: null };
  }

  if (route === "leaderboard") return { tab: "community", projectId: null, activitySection: null, communitySection: "leaderboard", itemId: null, workspace: null };
  if (route === "community") return { tab: "community", projectId: null, activitySection: null, communitySection: null, itemId: null, workspace: null };
  if (route === "profile") return { tab: "profile", projectId: null, activitySection: null, communitySection: null, itemId: null, workspace: null };
  if (route === "admin") return { tab: "profile", projectId: null, activitySection: null, communitySection: null, itemId: null, workspace: "admin" };
  if (route === "leader") return { tab: "profile", projectId: null, activitySection: null, communitySection: null, itemId: null, workspace: "leader" };
  if (route === "home") return { tab: "home", projectId: null, activitySection: null, communitySection: null, itemId: null, workspace: null };
...
function renderTab(
  ...
  if (tab === "home") {
    ...
    return <HomeScreen user={user} onOpenEvents={() => onTabChange("events")} onOpenCommunity={() => onTabChange("community")} />;
  }
...
export function App() {
  ...
  if (adminWorkspaceRequested) {
    return <AdminLayout onExitWorkspace={exitWorkspace}>{initialProjectId ? <ProjectsScreen initialProjectId={initialProjectId} /> : <AdminScreen />}</AdminLayout>;
  }
