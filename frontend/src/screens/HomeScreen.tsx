...
export function HomeScreen({
  ...
  const { data } = home;
  const growthPercent = data.growth.level_count <= 1
    ? 1
  ...
  return (
    ...
    >
      <h1 style={{ margin: 0, fontSize: "clamp(1.55rem, 7vw, 2rem)", lineHeight: 1.08 }}>
        {fullName}, держим темп
      </h1>
      ...
      <header style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
        ...
        >
          <strong style={{ display: "block", fontSize: "1.05rem" }}>{fullName}</strong>
          <span
            style={{
              display: "block",
              marginTop: 2,
          ...
          <div style={{ position: "relative", width: 104, height: 104, flexShrink: 0 }}>
            ...
            >
              <div>
                ...
                <span style={{ color: "var(--era-text-muted)", fontSize: ".68rem" }}>
                  до уровня
                </span>
              </div>
            </div>
      ...
      {onOpenDevelopment && (
        <Card
          ...
          style={{ padding: "1.1rem", borderLeft: "3px solid var(--era-red)" }}
        >
          <small style={{ color: "var(--era-red)", fontWeight: 800, letterSpacing: ".07em" }}>
            МОЙ ВЕКТОР
          </small>
          <strong style={{ display: "block", marginTop: 5, fontSize: "1.08rem" }}>
            Как ты изменился за последний месяц?
          </strong>
          <span style={{ display: "block", marginTop: 5, color: "var(--era-text-muted)" }}>
            Check-in и исследования · можно продолжить позже
          </span>
        </Card>
      )}
      ...
      <section style={{ display: "flex", flexDirection: "column", gap: ".75rem" }}>
        <div>
          ...
          <p style={{ margin: ".25rem 0 0", color: "var(--era-text-muted)" }}>
            Только то, что требует вашего внимания.
          </p>
        </div>
        <div
          style={{
        ...
        >
          <TodayMetric
            ...
            onClick={todayEventCount ? onOpenEvents : undefined}
          />
          <TodayMetric
            label="Задания"
        ...
        {todayEventCount === 0 && onOpenEvents && (
          <button type="button" onClick={onOpenEvents} style={{ width: "100%" }}>
              ...
              <Card onClick={() => onOpenTask(data.active_task!.id)}>
                <div style={{ display: "flex", gap: ".75rem" }}>
                  ...
                  <div>
                    ...
                    <p style={{ margin: ".25rem 0 0", color: "var(--era-text-muted)" }}>
                      До {formatDate(data.active_task.deadline)} · {data.active_task.points} баллов
                    </p>
                  </div>
                </div>
              </Card>
      ...
      <section style={{ display: "flex", flexDirection: "column", gap: ".75rem" }}>
        ...
        >
          ...
          {onOpenCommunity && (
            <button
              ...
              style={{ minHeight: 44, padding: ".5rem .8rem" }}
            >
              Все
            </button>
          )}
        ...
        {data.opportunities.length ? (
          data.opportunities.slice(0, 3).map((item) => (
            <Card
              key={item.id}
              onClick={onOpenOpportunity ? () => onOpenOpportunity(item.id) : onOpenCommunity}
            >
              <div style={{ display: "flex", gap: ".75rem" }}>
                <IconBubble tone="gold">
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <p style={{ margin: ".25rem 0 0", color: "var(--era-text-muted)" }}>
                    {item.point_cost ? `${item.point_cost} баллов` : "Доступно участникам"}
                    {item.expires_at ? ` · до ${formatDate(item.expires_at)}` : ""}
                  </p>
                </div>
              </div>
            </Card>
...
function TodayMetric({
    ...
    <>
      <strong
        style={{
          display: "block",
          fontSize: "1.45rem",
          color: value ? "var(--era-red)" : "var(--era-text)",
        }}
      >
        {value}
      </strong>
  ...
  return (
    <Card onClick={onClick} style={{ padding: ".8rem", textAlign: "center", boxShadow: "none" }}>
      {content}
    </Card>
  );
}
