...
export function AdminOverviewScreen({
  ...
  const scrollToAnalytics = () => document.getElementById("admin-analytics")?.scrollIntoView({ behavior: "smooth", block: "start" });
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.15rem" }}>
      <Card gradient style={{ position: "relative", overflow: "hidden", minHeight: 172 }}>
        ...
        <div style={{ position: "relative" }}>
          ...
          <h1 style={{ margin: 0, fontSize: "clamp(1.75rem,7vw,2.35rem)", lineHeight: 1.04 }}>ЭРА сегодня — сразу вся картина</h1>
      ...
      <section aria-label="Ключевые показатели">
        <h2 style={{ fontSize: ".86rem", color: "var(--era-text-muted)", margin: "0 0 .55rem" }}>КЛЮЧЕВЫЕ ПОКАЗАТЕЛИ</h2>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: ".55rem" }}>
          ...
          <KpiButton value={taskResults} label="На проверке" onClick={() => onOpenMetric("task_results", taskResults)} />
        </div>
      </section>

      <section id="admin-analytics" style={{ scrollMarginTop: "1rem" }}>
        <AdminDashboardScreen />
      </section>

      <section aria-label="Техническое состояние">
        <button type="button" onClick={() => setShowSystemDetails((value) => !value)} style={{ width: "100%", border: 0, padding: 0, background: "transparent", textAlign: "left" }}>
          <Card style={{ padding: ".8rem 1rem" }}>
            ...
            {system.status === "ready" && !latestHealth && <strong>Техническое состояние: нет данных · показать диагностику ↓</strong>}
            {latestHealth && healthIssues.length === 0 && <div style={{ display: "flex", justifyContent: "space-between", gap: ".75rem", alignItems: "center" }}><strong>Система в порядке · {showSystemDetails ? "свернуть" : "подробнее"}</strong><span style={{ color: "var(--era-success)", fontWeight: 850 }}>{latestHealth.score}/100 ✓</span></div>}
            {latestHealth && healthIssues.length > 0 && <div><strong style={{ color: "var(--era-error)" }}>{healthIssues.length} системных проблем · {latestHealth.score}/100</strong><span style={{ display: "block", marginTop: ".25rem", color: "var(--era-text-muted)", fontSize: ".76rem" }}>{healthIssues.slice(0, 3).map((item) => item.title).join(" · ")}{healthIssues.length > 3 ? " · …" : ""} · {showSystemDetails ? "свернуть ↑" : "подробнее ↓"}</span></div>}
          </Card>
        </button>
        {showSystemDetails && <div style={{ marginTop: ".65rem" }}><SystemPanel /></div>}
      </section>

      <ActionCell title="Проверка актуального состава" description="Community Verification: запуск, напоминания, недоступные и ручные решения" leading="◎" onClick={onOpenVerification} />
