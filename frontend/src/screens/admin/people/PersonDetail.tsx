...
function InfoRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "minmax(7rem, .9fr) minmax(0, 1.5fr)", gap: "0.75rem", padding: "0.5rem 0", borderBottom: "1px solid var(--era-border)" }}>
...
function Metric({ value, label }: { value: string | number; label: string }) {
  return (
    <div style={{ padding: "0.8rem", border: "1px solid var(--era-border)", borderRadius: "0.85rem", background: "var(--era-bg)" }}>
...
function ActivityList({ items, empty }: { items: ActivityRecord[]; empty: string }) {
  if (items.length === 0) return <p style={{ color: "var(--era-text-muted)", margin: 0 }}>{empty}</p>;
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.55rem" }}>
      {items.map((item) => (
        <div key={`${item.id}-${item.title}-${item.date ?? ""}`} style={{ padding: "0.75rem", border: "1px solid var(--era-border)", borderRadius: "0.8rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: "0.65rem", alignItems: "flex-start" }}>
            ...
            {item.points !== null && item.points !== undefined && (
              <span style={{ fontWeight: 800, whiteSpace: "nowrap" }}>{item.points > 0 ? "+" : ""}{item.points}</span>
            )}
          </div>
          {(item.subtitle || item.status || item.date) && (
            <p style={{ margin: "0.3rem 0 0", color: "var(--era-text-muted)", fontSize: "0.75rem", lineHeight: 1.45 }}>
              {[item.subtitle, item.status ? prettyStatus(item.status) : null, item.date ? formatDate(item.date) : null].filter(Boolean).join(" · ")}
            </p>
          )}
        </div>
      ))}
...
export function PersonDetail({ userId, onBack }: PersonDetailProps) {
  ...
  if (state.status === "loading") return <p style={{ color: "var(--era-text-muted)" }}>Загрузка полной карточки участника…</p>;
  if (state.status === "error") return <EmptyState text="Не удалось загрузить участника." />;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.85rem", minWidth: 0 }}>
      ...
      <Card>
        <div style={{ display: "flex", gap: "0.9rem", alignItems: "center", flexWrap: "wrap" }}>
          {person.photo_data_url ? (
            <img src={person.photo_data_url} alt="Фото участника" style={{ width: "5.75rem", height: "5.75rem", objectFit: "cover", borderRadius: "1.15rem", border: "1px solid var(--era-border)" }} />
          ) : (
            <div style={{ width: "5.75rem", height: "5.75rem", borderRadius: "1.15rem", display: "grid", placeItems: "center", background: "var(--era-bg)", border: "1px solid var(--era-border)", color: "var(--era-text-muted)", fontSize: "0.75rem", textAlign: "center", padding: "0.35rem", boxSizing: "border-box" }}>
              {person.photo_attached ? "Фото временно не загрузилось" : "Фото не загружено"}
            </div>
          )}
          <div style={{ flex: "1 1 12rem", minWidth: 0 }}>
            <p style={{ margin: 0, color: "var(--era-text-muted)", fontSize: "0.75rem", fontWeight: 800, textTransform: "uppercase" }}>Карточка участника</p>
            <h2 style={{ margin: "0.2rem 0 0", fontSize: "1.35rem", overflowWrap: "anywhere" }}>{person.first_name} {person.last_name ?? ""}</h2>
            <p style={{ margin: "0.3rem 0 0", color: "var(--era-text-muted)", fontSize: "0.8125rem" }}>{person.username ? `@${person.username}` : `Telegram ID ${person.telegram_id}`} · #{person.id}</p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: "0.4rem" }}>
        {tabs.map((tab) => (
          <button key={tab.key} type="button" onClick={() => setView(tab.key)} className={view === tab.key ? "era-btn-primary" : undefined} style={{ padding: "0.65rem 0.35rem", fontSize: "0.75rem" }}>
            {tab.label}
          </button>
        ))}
      </div>
          ...
          <Card>
            ...
            {person.leadership.strengths.length > 0 && (
              <div style={{ marginTop: "0.8rem" }}>
                <span style={{ fontSize: "0.75rem", fontWeight: 800, color: "var(--era-text-muted)", textTransform: "uppercase" }}>Подтверждённые сильные сигналы</span>
                {person.leadership.strengths.map((item) => <p key={item} style={{ margin: "0.4rem 0 0", fontSize: "0.875rem" }}>✓ {item}</p>)}
              </div>
            )}
            {person.leadership.growth_areas.length > 0 && (
              <div style={{ marginTop: "0.8rem" }}>
                <span style={{ fontSize: "0.75rem", fontWeight: 800, color: "var(--era-text-muted)", textTransform: "uppercase" }}>Что стоит развить</span>
                {person.leadership.growth_areas.map((item) => <p key={item} style={{ margin: "0.4rem 0 0", fontSize: "0.875rem" }}>→ {item}</p>)}
              </div>
            )}
          </Card>
            ...
            {person.social_links.length > 0 && (
              <div style={{ marginTop: "0.8rem" }}>
                ...
                {person.social_links.map((link) => (
                  <a key={`${link.platform}:${link.url}`} href={link.url} target="_blank" rel="noreferrer" style={{ display: "block", marginTop: "0.35rem", fontSize: "0.8125rem", overflowWrap: "anywhere" }}>{link.platform}: {link.url}</a>
                ))}
              </div>
            )}
          </Card>
      ...
      {view === "surveys" && (
        <Card>
          ...
          <p style={{ margin: "0.35rem 0 0", color: "var(--era-text-muted)", fontSize: "0.8125rem" }}>Здесь можно увидеть интересы, ожидания и обратную связь человека в динамике.</p>
        ...
        <>
          {person.can_manage && (
            <Card>
              ...
              <p style={{ margin: "0.35rem 0 0", color: "var(--era-text-muted)", fontSize: "0.8125rem" }}>Роль определяет место человека в системе. Точечные права ниже можно выдавать отдельно.</p>
              <select value={person.role} disabled={busy} onChange={(event) => runAction(() => changeUserRole(userId, event.target.value))} style={{ ...inputStyle, marginTop: "0.7rem" }}>
                {ROLE_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
              </select>
              <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.6rem", flexWrap: "wrap" }}>
                <button type="button" disabled={busy} onClick={() => runAction(() => setUserBlocked(userId, !person.is_blocked))}>{person.is_blocked ? "Разблокировать" : "Заблокировать"}</button>
                <button type="button" disabled={busy} onClick={() => runAction(() => setUserArchived(userId, !person.is_archived))}>{person.is_archived ? "Вернуть из архива" : "В архив"}</button>
              </div>
            </Card>
          )}
          ...
          {person.can_manage_permissions && (
            <Card>
              ...
              <p style={{ margin: "0.35rem 0 0", color: "var(--era-text-muted)", fontSize: "0.8125rem" }}>Каждое право выдаётся отдельно. Перед включением видно, что именно человек сможет делать.</p>
              <div style={{ display: "flex", flexDirection: "column", gap: "0.55rem", marginTop: "0.75rem" }}>
                {Object.entries(person.permissions).map(([permission, enabled]) => (
                  <div key={permission} style={{ padding: "0.75rem", border: `1px solid ${enabled ? "var(--era-gold)" : "var(--era-border)"}`, borderRadius: "0.85rem" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", gap: "0.75rem", alignItems: "flex-start" }}>
                      <div>
                        <strong style={{ fontSize: "0.875rem" }}>{PERMISSION_LABELS[permission] ?? permission}</strong>
                        <p style={{ margin: "0.25rem 0 0", color: "var(--era-text-muted)", fontSize: "0.75rem", lineHeight: 1.45 }}>{PERMISSION_DESCRIPTIONS[permission] ?? "Дополнительное управленческое право."}</p>
                      </div>
                      <button type="button" disabled={busy} onClick={() => runAction(() => toggleUserPermission(userId, permission))} className={enabled ? "era-btn-primary" : undefined} style={{ flexShrink: 0 }}>{enabled ? "Выдано" : "Выдать"}</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <Card>
              ...
              {person.points_suggestion && (
                <div style={{ marginTop: "0.7rem", padding: "0.8rem", borderRadius: "0.85rem", border: "1px solid var(--era-gold)", background: "var(--era-bg)" }}>
                  ...
                  <button type="button" className="era-btn-primary" disabled={busy} onClick={() => runAction(() => awardUserPoints(userId, person.points_suggestion!.amount, person.points_suggestion!.reason))} style={{ marginTop: "0.6rem" }}>Начислить предложенные баллы</button>
                </div>
              )}
              ...
              <button type="button" className="era-btn-primary" disabled={busy || !pointsAmount || !pointsReason.trim()} onClick={() => runAction(async () => { await awardUserPoints(userId, Number(pointsAmount), pointsReason.trim()); setPointsAmount(""); setPointsReason(""); })} style={{ marginTop: "0.55rem" }}>Применить вручную</button>
            </Card>
          )}
            ...
            <Card>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.55rem", marginTop: "0.45rem" }}>
                    {person.badge_suggestions.map((suggestion) => (
                      <div key={suggestion.badge_id} style={{ padding: "0.8rem", border: "1px solid var(--era-gold)", borderRadius: "0.85rem" }}>
                        ...
                        <button type="button" disabled={busy} onClick={() => runAction(() => awardUserBadge(userId, suggestion.badge_id, suggestion.reason))} style={{ marginTop: "0.55rem" }}>Выдать знак</button>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {availableBadges.length > 0 && (
                <div style={{ marginTop: "1rem", paddingTop: "0.8rem", borderTop: "1px solid var(--era-border)" }}>
                  ...
                  {selectedBadge && <p style={{ margin: "0.35rem 0 0", color: "var(--era-text-muted)", fontSize: "0.75rem" }}>{selectedBadge.description || BADGE_DESCRIPTIONS[selectedBadge.name] || "Знак за подтверждённый вклад участника."}</p>}
                  <input type="text" placeholder="За что выдаём этот знак?" value={badgeReason} onChange={(event) => setBadgeReason(event.target.value)} style={{ ...inputStyle, marginTop: "0.5rem" }} />
                  <button type="button" className="era-btn-primary" disabled={busy || !selectedBadge || !badgeReason.trim()} onClick={() => selectedBadge && runAction(async () => { await awardUserBadge(userId, selectedBadge.id, badgeReason.trim()); setSelectedBadgeId(""); setBadgeReason(""); })} style={{ marginTop: "0.5rem" }}>Выдать выбранный знак</button>
                </div>
              )}
            </Card>
          )}
