...
export interface VectorCheckin {
  ...
  answers: Partial<Record<VectorDimension, number>>;
...
export interface DevelopmentGoal {
  ...
  status: string;
  is_custom: boolean;
  review: { result: string; obstacle: string | null; note: string | null } | null;
}

export interface DevelopmentHome {
  title: string;
  subtitle: string;
  consent_required: boolean;
  consent_version: string;
  profile: VectorProfile | null;
  current_checkin: VectorCheckin | null;
  current_goal: DevelopmentGoal | null;
  questions: VectorQuestion[];
  answer_options: VectorOption[];
  context_options: string[];
  development_wants: string[];
  state_labels: Record<VectorDimension, string>;
}
...
export interface AssessmentCard {
  ...
  min_age: number | null;
  recommended_retake_after_days: number | null;
...
export interface DevelopmentPrivacy {
  consent_version: string;
  admin_visibility: { summary: boolean; interests: boolean; goals: boolean };
  admin_can_see: string[];
  private_only: string[];
}
